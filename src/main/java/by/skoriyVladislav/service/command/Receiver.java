    ...
    public void action(TypeCommand cmd, HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        try {
            ...
            switch (cmd) {
                ...
                case GO_TO_MAKE_BET:
                    Match match = DAOFactory.getInstance().getMatchDAO().getMatch(Integer.parseInt(request.getParameter("match")));
                    User user6 = (User) request.getSession().getAttribute("user");
                    Bet bet1 = DAOFactory.getInstance().getBetDAO().getBet(user6.getLogin(), match.getId());
                ...
                case SET_RESULT:
                    ...
                    int goalsTeam11 = Integer.valueOf(request.getParameter("goalsTeam1"));
                    int goalsTeam21 = Integer.valueOf(request.getParameter("goalsTeam2"));
                    DAOFactory.getInstance().getMatchDAO().setResult(matchId1, goalsTeam11, goalsTeam21);
                ...
                case GO_TO_MY_BETS:
                    ...
                    List<Bet> bets = DAOFactory.getInstance().getBetDAO().getBet(user1.getLogin());

                    List<Match> matches1 = new ArrayList<>();
                    for (Bet bet : bets) {
                        matches1.add(DAOFactory.getInstance().getMatchDAO().getMatch(bet.getIdMatches()));
                    }
                ...
                case GO_TO_USER_PROFILE:
                    ...
                    User user4 = DAOFactory.getInstance().getUserDAO().getUser(login4);
                ...
                case CHANGE_BET:
                    User user8 = (User) request.getSession().getAttribute("user");
                    Bet bet2 = (Bet) request.getSession().getAttribute("bet");

                    try {
                        if (DAOFactory.getInstance().getBetDAO().deleteBet(user8, bet2)) {
                            request.getSession().setAttribute("user", user8);
                            request.setAttribute("bet", null);
                            request.getSession().setAttribute("bet", null);
                            request.getRequestDispatcher("/controller?command=go_to_make_bet&match=" + bet2.getIdMatches()).forward(request, response);
                        } else {
                            request.getRequestDispatcher("error.jsp").forward(request, response);
                        }
                    ...
                    String userLogin1 = user8.getLogin();
                    BigDecimal size1 = BigDecimal.valueOf(Double.valueOf(request.getParameter("betVal")));

                    if (DAOFactory.getInstance().getUserDAO().checkBalanceForBet(userLogin1, size1)) {
                        int matchId = ((Match) request.getSession().getAttribute("match")).getId();
                        BetType typeOfBet = BetType.valueOf(request.getParameter("betType").toUpperCase());
                        Integer goalsTeam1 = null;
                        Integer goalsTeam2 = null;
                        ...
                        Bet bet3 = new Bet(userLogin1, matchId, size1, typeOfBet, goalsTeam1, goalsTeam2, null);

                        DAOFactory.getInstance().getBetDAO().registrationBet(bet3, user8);

                        request.getSession().setAttribute("user", user8);
                        request.getSession().setAttribute("bet", bet3);
                        request.setAttribute("bet", bet3);
                        request.getRequestDispatcher("/WEB-INF/jsp/make_bet.jsp").forward(request, response);
                    } else {
                        response.sendRedirect("error.jsp");
                    }
                ...
                case CHANGE_COEFF:
                    ...
                    DAOFactory.getInstance().getMatchDAO().changeCoefficients(idMatch, coeff);
                    break;
                ...
                case MAKE_MATCH:
                    ...
                    if (validateString(team1, team2, year, month, day, hour, minute)) {
                        ...
                        DAOFactory.getInstance().getMatchDAO().registrationMatch(team1, team2, dataTime, coef);
                ...
                case LOGIN:
                    ...
                    User user2 = userDAO1.getUser(login1, password1);
                ...
                case CHECK_TIME_AJAX:
                    ...
                    try (PrintWriter out = response.getWriter()) {
                        JSONObject jsonEnt = new JSONObject();
                        if (DAOFactory.getInstance().getMatchDAO().checkTimeForBet(Integer.valueOf(request.getParameter("matchId")))) {
                            jsonEnt.put("serverInfo", "true");
                        } else {
                            jsonEnt.put("serverInfo", "false");
                        }
                ...
                case GET_USERS_AJAX:
                    ...
                    List<User> products = DAOFactory.getInstance().getUserDAO().getUsers(criteria);
