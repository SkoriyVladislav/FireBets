...
public class UserServiceImpl implements UserService {
    ...
    public User getUser(String login, String pass) throws ServiceException {
        if (ServiceValidator.isValidSting(login) && ServiceValidator.isValidSting(pass)) {
            try {
    ...
    public User getUser(String login) throws ServiceException {
        if (ServiceValidator.isValidSting(login)) {
            try {
    ...
    public boolean registerUser(String login, String password, String name, String surname, String role, BigDecimal balance, String email) throws ServiceException {
        if (ServiceValidator.isValidSting(login) && ServiceValidator.isValidSting(password) && ServiceValidator.isValidName(name) &&
                ServiceValidator.isValidName(surname) && ServiceValidator.isValidSting(role) && ServiceValidator.isValidSting(email) &&
    ...
    public boolean checkBalanceForBet(String login, BigDecimal size) throws ServiceException {
        if (ServiceValidator.isValidSting(login) && size.compareTo(BigDecimal.ZERO) >= 0) {
            try {
    ...
    public boolean changeRole(String login, String role) throws ServiceException {
        if (ServiceValidator.isValidSting(login) && ServiceValidator.isValidSting(login)) {
                try {
